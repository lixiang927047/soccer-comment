...
from videollava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    ...
    MAX_VIDEO_LENGTH, DEFAULT_AUDIO_TOKEN, DEFAULT_AUDIO_PATCH_TOKEN, DEFAULT_AUD_END_TOKEN, DEFAULT_AUD_START_TOKEN
...
from videollava.mm_utils import tokenizer_image_token, tokenizer_multimodal_token
...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            ...
            # ======================================================================================================
            # if sentence['value'].startswith(DEFAULT_AUDIO_TOKEN) or sentence['value'].startswith(
            ...
            if data_args.mm_use_im_start_end:
                audio_replace_token = DEFAULT_AUDIO_START_TOKEN + replace_token + DEFAULT_AUDIO_END_TOKEN
...
def preprocess(
    ...
    tokenizer: transformers.PreTrainedTokenizer,
    has_image: bool = False,
    has_audio: bool = False   ########## add audio #########
) -> Dict:
    """
    ...
    if conversation_lib.default_conversation.version.startswith("v1"): # flow to here
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0] and 'video' not in sources[0]:
            ...
            elif 'image' not in sources[0] and 'video' in sources[0]: # and ('audio' not in sources[0] or sources[0]['audio'] is None):  # add audio
                # rank0_print('video')
            ...
            elif 'image' not in sources[0] and 'video' in sources[0] and 'audio' in sources[0]: # and sources[0]['audio'] is not None:  # add audio
                # rank0_print('audio')
                video_file = self.list_data_dict[i]['video']
                video_folder = self.data_args.video_folder
                video_processor = self.data_args.video_processor
                ...
                audio_processer = self.data_args.audio_processor
                ...
                audio = [audio_processer(i, return_tensors='pt')['pixel_values'][0] for i in audio]  # fake image
            ...
            elif 'image' in sources[0] and 'video' in sources[0]:  # image和video都存在的情况
...
def train():
    ...
    # ==========================================================================
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained( # flow to here
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                **bnb_model_from_pretrained_args
            )
