...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if getattr(config, "mm_audio_tower", None) is not None:
            ...
            self.audio_mm_projector = build_vision_projector(config)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # ==========================================================================
        if image_tower is not None and video_tower is not None and audio_tower is not None:  # TODO: support different hidden_size
        ...
        if getattr(self, 'mm_projector', None) is None:
            self.mm_projector = build_vision_projector(self.config)  # requires_grad=True
            self.audio_mm_projector = build_vision_projector(self.config)  ## add audio
        else:
            # In case it is frozen by LoRA
            for p in self.mm_projector.parameters():
                p.requires_grad = True
            for p in self.audio_mm_projector.parameters():   ## add audio
                p.requires_grad = True
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            self.audio_mm_projector.load_state_dict(get_w(mm_projector_weights, 'mm_projector'))   # add audio
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_audios(self, audios):  # [mini_b, c, t, h, w]
        audio_features = self.get_model().get_audio_tower()(audios)  # [mini_b, t, n, c]
        audio_features = self.get_model().audio_mm_projector(audio_features)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        '''
        image_idx = [idx for idx, img in enumerate(images) if img.ndim == 3 and img.shape[-1] == 224]
        is_all_image = len(image_idx) == len(images)
        video_idx = [idx for idx, vid in enumerate(images) if vid.ndim == 4]
        audio_idx = [idx for idx, aud in enumerate(images) if aud.ndim == 3 and aud.shape[-1] != 224]  # audio has a specific shape [3, 126, 1036]
        images_minibatch = torch.stack([images[idx] for idx in image_idx]) if len(image_idx) > 0 else []  # mini_b c h w  [24, 3, 224, 224]
        videos_minibatch = torch.stack([images[idx] for idx in video_idx]) if len(video_idx) > 0 else []  # mini_b c t h w
        audios_minibatch = torch.stack([images[idx] for idx in audio_idx]) if len(audio_idx) > 0 else []  # [1, 3, 126, 1036]

        # 对 images 和 videos 进行encode 和 align
        tmp_image_features = [None] * (len(image_idx) + len(video_idx) + len(audio_idx))
        if getattr(audios_minibatch, 'ndim', 0) == 4 and getattr(audios_minibatch, 'shape')[-1] != 224:  # batch consists of images, [mini_b, c, h, w]
            if audio_tower is not None:
                audio_features_minibatch = self.encode_audios(audios_minibatch)  # [mini_b, l, c]
            else:
        ...
        if getattr(images_minibatch, 'ndim', 0) == 4 and getattr(images_minibatch, 'shape')[-1] == 224:  # batch consists of images, [mini_b, c, h, w]
            if image_tower is not None:
                image_features_minibatch = self.encode_images(images_minibatch)  # [mini_b, l, c]  # [24, 256, 4096]
            else:
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b, c, t, h, w]
            video_features_minibatch = self.encode_videos(videos_minibatch)  # fake list [mini_b, t, l, c]  # 1, 8, 257, 4096]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # 仅 encode 文本特征
            # 添加 audio 的 token_index
            image_token_indices = [-1] + torch.where((cur_input_ids == IMAGE_TOKEN_INDEX) | (cur_input_ids == AUDIO_TOKEN_INDEX))[0].tolist() + [cur_input_ids.shape[0]]
            cur_input_ids_noim = []
            cur_labels = labels[batch_idx]
            cur_labels_noim = []
            for i in range(len(image_token_indices) - 1):# 将被每个IMAGE_TOKEN_INDEX隔开的前后，都分别append进 cur_input_ids_noim，cur_labels_noim
